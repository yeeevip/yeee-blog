<template>
  <el-dialog title="详情"  class="mod-logLogin-info"
    :close-on-click-modal="false" :visible.sync="visible">
    <el-form :model="dataForm" ref="dataForm" label-width="120px">
      <el-row :gutter="20">
        <el-col :span="12">
          <el-form-item label="类型:">
            <label>{{ dataForm.type === 0 ? '登陆' : '登出' }}</label>
          </el-form-item>
        </el-col>
        <el-col :span="12">
...
<script>
  export default {
    ...
    methods: {
      ...
      // 获取系统日志信息
      getInfo () {
        this.$http.get('/sys/logLogin/info/' + this.id).then(({data: res}) => {
          if (res.code !== 0) {
