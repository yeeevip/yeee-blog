<template>
  <el-dialog :title="!id ? '新增' : '修改'" class="mod-classify-edit"
...
export default {
  ...
  methods: {
    ...
    // 获取信息
    getInfo () {
      this.$http.json().post('/manage/blog/classify/info', {
        'id': this.id
      }).then(({data: res}) => {
        if (res.code !== 200) {
    ...
    dataFormSubmit: debounce(function () {
      ...
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          this.$http.json().post(!this.dataForm.id ? '/manage/blog/classify/add' : '/manage/blog/classify/upd', {...this.dataForm}).then(({data: res}) => {
            if (res && res.code === 200) {
              this.$message({
                message: '操作成功',
                type: 'success',
