...
<!--    <div class="blank"></div>-->
    ...
    <div class="blogsbox">
      ...
      >
        ...
        <p class="blogtext">{{item.remark}}</p>
        <div class="bloginfo">
          <ul>
  ...
  export default {
    ...
    data() {
      return {
        ...
        VUE_YEEE_WEB: process.env.VUE_YEEE_WEB,
        firstData: [], //；一级推荐数据
        secondData: [], //；二级级推荐数据
        thirdData: [], //三级推荐
        fourthData: [], //四级推荐
    ...
    created() {
      ...
      //   pageSize: 5
      // };
      // getBlogByLevel(JSON.stringify(secondParams)).then(response => {
      //   if(response.code == this.$ECode.SUCCESS) {
      //     this.secondData = response.data.result;
      //   }
      // });
      // 获取最新博客
      this.newBlogList();
    ...
    methods: {
      ...
      newBlogList() {
        ...
        getNewBlog(JSON.stringify(params)).then(response => {
          if (response.code == this.$ECode.SUCCESS) {
            that.newBlogData = response.data.result;
      ...
      loadContent: function () {
        ...
        getNewBlog(JSON.stringify(params)).then(response => {
          if (response.code == this.$ECode.SUCCESS && response.data.result.length > 0) {
            that.isEnd = false;
            var newData = that.newBlogData.concat(response.data.result);
