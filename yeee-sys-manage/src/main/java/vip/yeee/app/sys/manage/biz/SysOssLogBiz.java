...
public class SysOssLogBiz {
    ...
    public Void sysOssLogAdd(SysOssLogAddRequest request) {
        SysOssLog save = new SysOssLog();
        save.setSourceName(request.getSourceName());
        save.setSize(request.getSize());
        save.setOssUrl(qiniuOssProperties.getDomain() + request.getOssUrl());
        sysOssLogService.sysOssLogAdd(save);
        return null;
    }
